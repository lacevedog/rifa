...
function on_duplicate($table = '', $set = NULL, $set_update = NULL )
{
    if ( ! is_null($set))
    {
        $this->set($set);
        $insert_values=$this->ar_set;
        
    }

    if(! is_null($set_update)){
    	unset($this->ar_set);
    	$this->set($set_update);
        $update_values=$this->ar_set;    
    }else{
 		$update_values=$this->ar_set;
    }
    ...
    $sql = $this->_duplicate_insert($this->_protect_identifiers($this->dbprefix.$table), $insert_values, $update_values);
